...
- `verify_refs.py`: `extract_refs(text)` pulls backticked path references out of the handoff prose; `verify_refs(refs, cwd)` stats each one against the working tree and reports whether the path still exists and whether any cited line number is still in range. This is how context-window-resume flags stale references before you choose a direction.
